...
// Components Import
import TechController from "@/app/(components)/(SkillSectComps)/TechController.jsx";
import Laptop from "@/app/(components)/(SkillSectComps)/Laptop.jsx";
import Features from "@/app/(components)/(SkillSectComps)/features.jsx";

// Data Import
import { webSkillsSect } from "../(utils)/data.js";
import { webSkillsSectPort } from "../(utils)/dataPortuguese.js";
...
function Skills() {
  ...
  const changeVisibility = (side: string | number, state: boolean, time: number = 0.5) => {
    return state
      ? {
          opacity: 1,
